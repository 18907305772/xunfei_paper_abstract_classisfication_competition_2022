...
class OurTrainingArguments(TrainingArguments):
    ...
    predict_top2: bool = field(
        default=False,
        metadata={"help": "是否预测时单独预测一部分 top2"}
    )
    @cached_property
    @torch_required
...
def construct_test_overlap_signal(raw_data_path="../xfdata/基于论文摘要的文本分类与查询性问答公开数据/"):
    train_df = pd.read_csv(raw_data_path + "train.csv", delimiter=',')
    test_df = pd.read_csv(raw_data_path + "test.csv", delimiter=',')
    leak_labels = []
    leak_num = 0
    for test_row in range(test_df.shape[0]):
        test_doi = test_df.loc[test_row, "DOI"]
        try:
            leak_row = train_df[train_df.DOI == test_doi].index.tolist()[0]
            leak_labels.append(train_df.loc[leak_row, "Topic(Label)"])
            leak_num += 1
        except:
            leak_labels.append(-1)
    return leak_labels
...
def main():
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            if training_args.predict_top2 is True:
                overlap_signal = construct_test_overlap_signal()
                pred_results = []
                top1_cnt, top2_cnt = 0, 0
                for row in range(predictions.shape[0]):
                    if overlap_signal[row] != -1:
                        if label_list[np.argmax(predictions[row])] != overlap_signal[row]:
                            pred_results.append(np.argmax(predictions[row]))
                            top1_cnt += 1
                        else:
                            pred_results.append(np.argsort(predictions[row])[-2])
                            top2_cnt += 1
                    else:
                        pred_results.append(np.argmax(predictions[row]))
                predictions = pred_results
                print("top1:{}, top2:{}".format(top1_cnt, top2_cnt))
            else:
