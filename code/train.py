...
class OurTrainingArguments(TrainingArguments):
    ...
    test_fake_label: bool = field(
        default=False,
        metadata={"help": "predict test label each fold"}
    )
    @cached_property
    @torch_required
...
def main():
    ...
    pad_ways = "maxlenpad" if data_args.pad_to_max_length else "batchpad"
    tricks = ""
    ...
    if training_args.lr_scheduler_type != "linear":
        tricks += "_{}-scheduler".format(training_args.lr_scheduler_type)
    ...
    if training_args.multi_sample_dropout_num > 1:
        ...
        tricks += "_multi_sample_drop{}_{}".format(training_args.multi_sample_dropout_num, avg_or_sum)
    ...
    training_args.output_dir += "{}_{}_gpunum{}_{}_bs{}_accumulate{}_lr{}_epoch{}{}/".format(
        data_args.train_file[5:-10].replace("/", "_"), training_args.train_valid_split, training_args.gpu_nums,
    ...
    # if "deberta" not in model_args.model_name_or_path.lower() and "spanbert" not in model_args.model_name_or_path.lower() and "bert" in model_args.model_name_or_path.lower():
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            if trainer.is_world_process_zero():
                if not os.path.exists(training_args.result_record_csv):
                    ...
                    csv_head = ["model", "dataset", "train valid fold", "max len", "pad to max len", "num_gpus",
                ...
                valid_record = [model_args.model_name_or_path.replace("/", "_"), data_args.train_file[5:-10].replace("/", "_"),
                                training_args.train_valid_split, data_args.max_seq_length, data_args.pad_to_max_length,
                                training_args.gpu_nums, training_args.per_device_train_batch_size,
                                training_args.gradient_accumulation_steps, training_args.learning_rate,
                                training_args.num_train_epochs, tricks, metrics["eval_accuracy"], " "]
    ...
    if training_args.test_fake_label is True:
        logger.info("*** Predict test fake label ***")

        predict_datasets = [predict_dataset]
        datasets_name = ["test"]
        datasets_file = [data_args.test_file]
        for idx, predict_dataset in enumerate(predict_datasets):
            predictions = trainer.predict(predict_dataset, metric_key_prefix="predict").predictions
            predictions = np.squeeze(predictions) if is_regression else np.argmax(predictions, axis=1)
            output_predict_file = training_args.output_dir + "{}_predict.csv".format(datasets_name[idx])
